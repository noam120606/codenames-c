...
/**
 * Teste si une chaîne de caractères correspond à une expression régulière compilée.
 * @param re Pointeur vers une structure CnRegex compilée.
 * @param text Chaîne de caractères à tester.
 * @return 0 si la chaîne correspond, 1 si elle ne correspond pas, -1 en cas d'erreur.
 */
int cn_regex_match(const CnRegex* re, const char* text);

/**
 * Libère les ressources associées à une expression régulière compilée.
 * Après l'appel de cette fonction, la structure CnRegex ne doit plus être utilisée.
